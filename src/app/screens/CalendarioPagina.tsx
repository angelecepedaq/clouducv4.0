...
// Formatear hora desde ISO timestamp
...
const CalendarioPagina: FC = () => {
  ...
  // Extraer días con eventos del mes actual a partir de start_date
  ...
  const eventosDia = diaSeleccionado
    ? eventos.filter((e) => {
        try {
          const d = new Date(e.start_date);
          return d.getDate() === diaSeleccionado && d.getMonth() === mes && d.getFullYear() === anio;
        } catch {
          return false;
        }
      })
            ...
            eventosDia.map((e) => (
              <div key={e.id} className="rounded-xl p-4 mb-3 ucv-card-bg flex gap-3 items-start">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-lavender text-xs mt-0.5">{formatHora(e.start_date)} · {e.category}</p>
                  {e.location && (
                    <p className="text-lavender text-xs mt-0.5">{e.location}</p>
                  )}
                </div>
              </div>
            ))
